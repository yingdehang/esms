...
$(document).ready(function(){
	...
	// alert(num.substr(1));
	turnplate.restaraunts = [ "象本钥匙扣", "落地加湿器", "象本抱枕", "谢谢参与", "象本钥匙扣","象本抱枕","落地加湿器","象本钥匙扣", "谢谢参与"];
	turnplate.colors = ["#ffcc40", "#ffda73", "#ffbb00", "#ffcc40","#ffda73", "#ffbb00","#ffcc40","#ffda73","#ffbb00"];
// turnplate.fontcolors = ["#CB0030", "#FFFFFF", "#CB0030","#FFFFFF","#CB0030",
// "#FFFFFF"];
...
// save();
	...
	$('#tupBtn').click(function (){
		...
		var isLack=$("#isLack").val();
		var isTime=$("#isTime").val();
		if(isTime != "活动进行中"){
			$(".xxcy_text").html(isTime);
			 $("#xxcy-main").fadeIn();
			 return;
		}
		if(isLack != 1){
			 $(".xxcy_text").html("您不符合抽奖条件");
			 $("#xxcy-main").fadeIn();
			 return;
		}
		if(gamed != "2"){
			 $(".xxcy_text").html("您的抽奖次数已用完<br/>每位用户只有一次抽奖机会");
			 $("#xxcy-main").fadeIn();
			 return;
		}
		...
		turnplate.bRotate = !turnplate.bRotate;
		 $.ajax({
			 type:"get",
			 async:false,
			 url:"/activityclick/queryPrizeInfo",
			 data:{"userId":userId},
			 dataType:"json",
			 ...
			 // 获取随机数(奖品个数范围内)
			 var item = data.index;
		 	// 奖品数量等于10,指针落在对应奖品区域的中心角度[252, 216, 180, 144, 108, 72, 36, 360,
			// 324, 288]
...
function drawRouletteWheel() {    
  ...
  if (canvas.getContext) {
	  ...
	  for(var i = 0; i < turnplate.restaraunts.length; i++) {       
		  ...
		  if(text.indexOf("\n")>0){// 换行
			  ...
			  for(var j = 0; j<texts.length; j++){
				  ...
				  if(j == 0){
					  	// ctx.fillText(texts[j]+"M",
						// -ctx.measureText(texts[j]+"M").width / 2, j *
						// line_height);
    ...
    /*
	 * var useragent = navigator.userAgent; if
	 * (useragent.match(/MicroMessenger/i) != 'MicroMessenger') { //
	 * 这里警告框会阻塞当前页面继续加载 alert('已禁止本次访问：您必须使用微信内置浏览器访问本页面！'); //
	 * 以下代码是用javascript强行关闭当前页面 var opened = window.open('about:blank',
	 * '_self'); opened.opener = null; opened.close(); }
	 */
